...
<div class="container">
    ...
    <div id="dashboardSection" class="hidden">
        ...
        <div id="adminPanelSection" class="card hidden" style="border: 2px solid var(--accent); background: #fffde7;">
            ...
            <div id="mainAdminCodeBox" class="hidden" style="background: #fff9c4; padding: 10px; border-radius: 6px; margin-bottom: 10px;">
            ...
            <!-- POINTS TABLE MANAGEMENT BOX -->
            <div style="background: #e0f2f1; padding: 10px; border-radius: 6px; margin-bottom: 15px; border: 1px solid var(--primary);">
                <h4 style="margin: 0 0 8px 0; color: var(--primary-dark);">🏆 Points Table Management (Group Wise)</h4>
                
                <label style="font-size: 13px; font-weight: bold; color: #333; display: block; margin-top: 5px;">Group Chunein:</label>
                <select id="ptGroupSelect">
                    <option value="">Pehle Group Chunein</option>
                </select>

                <label style="font-size: 13px; font-weight: bold; color: #333; display: block; margin-top: 8px;">Team Chunein:</label>
                <select id="ptTeamSelect">
                    <option value="">Pehle Group Chunein</option>
                </select>

                <div style="display: flex; gap: 5px;">
                    <input type="number" id="ptPlayedInput" placeholder="P (Played)" style="margin-top:8px;">
                    <input type="number" id="ptWonInput" placeholder="W (Won)" style="margin-top:8px;">
                    <input type="number" id="ptLostInput" placeholder="L (Lost)" style="margin-top:8px;">
                </div>
                <div style="display: flex; gap: 5px;">
                    <input type="number" id="ptNrInput" placeholder="NR (No Result)" style="margin-top:8px;">
                    <input type="text" id="ptNrrInput" placeholder="NRR (e.g. +1.245)" style="margin-top:8px;">
                    <input type="number" id="ptPointsInput" placeholder="Points (Pts)" style="margin-top:8px;">
                </div>
                <button class="action-btn" id="savePointTableTeamBtn" style="margin-top: 10px; background: var(--primary-dark);">Team Points Table Update Karein</button>
            ...
            <button class="action-btn" id="updateResultBtn" style="background: #0284c7;">Result Update Karein</button>
...
<script>
    ...
    function populateAdminGroupDropdown() {
        ...
        document.getElementById('ptTeamSelect').innerHTML = "<option value=''>Pehle Group Chunein</option>";
    }

    function onAdminGroupSelected() {
        let groupName = document.getElementById('ptGroupSelect').value;
        let groups = getGroups();
        let teamSelect = document.getElementById('ptTeamSelect');

        if(!groupName || !groups[groupName]) {
            teamSelect.innerHTML = "<option value=''>Pehle Group Chunein</option>";
            return;
        }

        let teams = groups[groupName];
        teamSelect.innerHTML = "<option value=''>-- Team Chunein --</option>";
        teams.forEach(team => {
            teamSelect.innerHTML += `<option value="${team}">${team}</option>`;
        });
    }

    function savePointTableTeam() {
        let tableName = document.getElementById('ptGroupSelect').value.trim(); // Group name hi table name banega
        let teamName = document.getElementById('ptTeamSelect').value.trim();
        let p = parseInt(document.getElementById('ptPlayedInput').value) || 0;
        let w = parseInt(document.getElementById('ptWonInput').value) || 0;
        let l = parseInt(document.getElementById('ptLostInput').value) || 0;
        let nr = parseInt(document.getElementById('ptNrInput').value) || 0;
        let nrr = document.getElementById('ptNrrInput').value.trim() || '0.000';
        let pts = parseInt(document.getElementById('ptPointsInput').value) || 0;
        ...
        tables[tableName][teamName] = { played: p, won: w, lost: l, nr: nr, nrr: nrr, points: pts };
        savePointsTables(tables);

        document.getElementById('ptPlayedInput').value = "";
        document.getElementById('ptWonInput').value = "";
        document.getElementById('ptLostInput').value = "";
        document.getElementById('ptNrInput').value = "";
        document.getElementById('ptNrrInput').value = "";
        document.getElementById('ptPointsInput').value = "";

        renderAllPointsTables();
        renderAdminPointsTablesList();
        alert("Team points table mein successfully update ho gayi hai!");
    }

    window.deleteTableTeam = function(tableName, teamName) {
        if(confirm(`Kya aap ${tableName} se ${teamName} ko hatana chahte hain?`)) {
            let tables = getPointsTables();
            if(tables[tableName]) {
                delete tables[tableName][teamName];
                if(Object.keys(tables[tableName]).length === 0) {
    ...
    function renderAllPointsTables() {
        ...
        keys.forEach(tName => {
            ...
            html += `<div class="card">
                ...
                <table>
                    <tr><th>Team</th><th>P</th><th>W</th><th>L</th><th>NR</th><th>Pts</th><th>NRR</th></tr>`;
            
            teamKeys.forEach(t => {
                let d = teamsObj[t];
                html += `<tr>
                    <td style="text-align:left; padding-left:5px;"><b>${t}</b> ${isAdminUnlocked ? `<button onclick="deleteTableTeam('${tName}', '${t}')" style="background:none; border:none; color:red; cursor:pointer; font-size:10px; float:right;" title="Delete Team">❌</button>` : ''}</td>
                    <td>${d.played}</td>
                    <td>${d.won}</td>
                    <td>${d.lost}</td>
                    <td>${d.nr || 0}</td>
    ...
    function renderGroups() {
        ...
        let html = "";
        keys.forEach(g => {
            html += `<div style="background:#f9f9f9; border:1px solid #ddd; padding:8px; border-radius:6px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;">
                <div><strong>📌 ${g}</strong><br><span style="font-size:13px; color:#555;">Teams: ${groups[g].join(', ')}</span></div>
                ${isAdminUnlocked ? `<button onclick="deleteGroup('${g}')" style="background:#dc3545; color:white; border:none; padding:4px 8px; border-radius:4px; font-size:11px; cursor:pointer; width:auto;">Delete</button>` : ''}
            </div>`;
        });
        container.innerHTML = html;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('savePointTableTeamBtn').addEventListener('click', savePointTableTeam);
